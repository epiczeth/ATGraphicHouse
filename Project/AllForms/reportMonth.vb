...
Namespace AllForms
    Public Class ReportMonth
        ...
        Private Sub reportMonth_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
            Dim sql As String
            sql =
                "SELECT Orders.Job_Id, Customer.Cus_Name, Orders.Job_Name, Orders.Job_Date, Orders.Job_Sent, Orders.Job_Price FROM (Customer INNER JOIN Orders ON Customer.Cus_Id = Orders.Cus_Id) WHERE  month(Orders.Job_Date)=" &
                mm & " AND year(Orders.Job_Date)=" & yy & " AND Orders.Job_Status='ชำระแล้ว'"

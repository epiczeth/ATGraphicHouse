...
Namespace AllForms
    Partial Public Class UserForm
        ...
        Private Sub Fill_dgv()
            Dim sql As String =
                    "SELECT Usertable.User_Id, Usertable.UserName, Usertable.Password, Usertable.UserLevel, Employee.Emp_Name "
        ...
        Private Function IsValidated() As Boolean
            If _
                cbEmpID.Text <> "" AndAlso cbEmpName.Text <> "" AndAlso txtusername.Text <> "" AndAlso
                txtuserid.Text <> "" AndAlso txtPassword.Text <> "" AndAlso cbuserlevel.Text <> "" Then
                Return True
            Else
                Return False
            End If
        ...
        Private Sub Button2_Click(ByVal sender As Object, ByVal e As EventArgs) Handles Button2.Click
            If IsValidated() Then
                _ds =
                    _con.Getdata(
                        "SELECT * FROM Usertable WHERE User_Id=" & txtuserid.Text & " OR UserName='" &
                        txtusername.Text.Trim() & "'")
                If _ds.Tables(0).Rows.Count <= 0 Then

                    Dim sql As String = String.Format("insert into Usertable values({0},'{1}','{2}','{3}',{4})",
                                                      Convert.ToInt32(txtuserid.Text), txtusername.Text,
                                                      txtPassword.Text, cbuserlevel.Text, Convert.ToInt32(cbEmpID.Text))
        ...
        Private Sub Button4_Click(ByVal sender As Object, ByVal e As EventArgs) Handles Button4.Click
            If dgv.SelectedRows.Count >= 0 Then
                ...
                Dim reader As MySqlDataReader =
                        (New MySqlCommand(
                            "SELECT * FROM Usertable WHERE User_Id=" & dgv.SelectedRows(0).Cells(0).Value.ToString() &
                            " ORDER BY User_Id", conn)).ExecuteReader() 'อ่านข้อมูลจากฐานข้อมูล
        ...
        Private Sub Button3_Click(ByVal sender As Object, ByVal e As EventArgs) Handles Button3.Click
            _ds =
                _con.Getdata(
                    "SELECT * FROM Usertable WHERE User_Id=" & Convert.ToInt32(dgv.SelectedRows(0).Cells(0).Value) &
                    " ORDER BY User_Id")
            If _ds.Tables(0).Rows.Count >= 1 Then
                If _
                    MessageBox.Show(Me,
                    ...
                    If _
                        _con.Query(
                            ...
                            Convert.ToInt32(dgv.SelectedRows(0).Cells(0).Value.ToString()) & "") >= 1 Then
        ...
        Private Sub Button5_Click(ByVal sender As Object, ByVal e As EventArgs) Handles Button5.Click
            If IsValidated() Then
                ...
                If _ds.Tables(0).Rows.Count = 1 Then
                    ...
                    sql &= "Emp_Id=" & Convert.ToInt32(cbEmpID.Text) & " WHERE User_Id=" &

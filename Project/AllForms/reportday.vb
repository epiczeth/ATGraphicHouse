...
Namespace AllForms
    Public Class Reportday
        ...
        Private Sub reportday_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
            ...
            Dim sql As String
            sql =
                "SELECT Orders.Job_Id, Customer.Cus_Name, Orders.Job_Name, Orders.Job_Date, Orders.Job_Sent, Orders.Job_Price FROM (Orders INNER JOIN Customer ON Orders.Cus_Id = Customer.Cus_Id) WHERE day(Orders.Job_Date)=" &
                dd & " and month(Orders.Job_Date)=" & mm & " and year(Orders.Job_Date)=" & yyyy &
                " AND (Orders.Job_Status = 'ชำระแล้ว') "
            Debug.WriteLine(sql)

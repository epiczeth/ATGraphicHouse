...
Namespace AllForms
    Partial Public Class Customer
        ...
        Public Sub Save()
            If Validation() Then
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    _sql = "update customer set cus_name ='" & cbCusName.Text & "', cus_address='" & txtCusAdd.Text &
                           "', cus_gov='" & txtCusGov.Text & "',cus_tel='" & txtCusTel.Text & "',cus_email='" &
                           txtCusEmail.Text & "' where cus_id=" & cbCusID.Text & ""
                Else
                    _sql = "insert into customer (cus_name, cus_address, cus_tel, cus_gov, cus_email)  "
                    _sql &= "values ('" & cbCusName.Text & "','" & txtCusAdd.Text & "','" & txtCusTel.Text & "','" &
                            txtCusGov.Text & "','" & txtCusEmail.Text & "')"
                End If
        ...
        Private Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
            If _
                MessageBox.Show(
                    "คุณต้องการลบ '" & dgv.SelectedRows(0).Cells(1).Value.ToString() & "' ออกจากฐานข้อมูลใช่หรือไม่?",
                    "กดยืนยันเพื่อลบ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then

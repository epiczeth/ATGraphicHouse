...
Namespace AllForms
    Partial Public Class Employees
        ...
        Public Sub Save()
            If Validation() Then
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    _sql = "update employee set emp_name ='" & txtEmpName.Text & "', emp_address='" & txtEmpAdd.Text &
                           "', emp_position='" & cmbPosition.Text & "',emp_salary=" & txtSalary.Text & " where emp_id =" &
                           txtEmpID.Text & " "

                Else
                    _sql = "insert into employee (emp_name, emp_address, emp_tel, emp_position, emp_salary)  "
                    _sql &= "values ('" & txtEmpName.Text & "','" & txtEmpAdd.Text & "','" & txtTel.Text & "','" &
                            cmbPosition.Text & "'," & txtSalary.Text & ")"
                End If
        ...
        Private Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
            If _
                MessageBox.Show("คุณต้องการลบ '" & dgv.SelectedRows(0).Cells(1).Value.ToString() & "' ใช่หรือไม่?",
                                "กดยืนยันเพื่อลบ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes _
                Then
        ...
        Private Sub txtSalary_KeyPress(ByVal sender As Object, ByVal e As KeyPressEventArgs) Handles txtSalary.KeyPress
            If Convert.ToInt32(e.KeyChar) >= 48 AndAlso Convert.ToInt32(e.KeyChar) <= 57 Then
                e.Handled = False
            ElseIf _
                (Convert.ToInt32(e.KeyChar) = 8) OrElse (Convert.ToInt32(e.KeyChar) = 13) OrElse
                (Convert.ToInt32(e.KeyChar) = 46) Then
                e.Handled = False
            Else
                e.Handled = True
            End If

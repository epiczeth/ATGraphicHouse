...
Namespace AllForms
    Partial Public Class FrmBill
        ...
        Public Shared CusId As Integer = 0
        ...
        Private Sub button1_Click(ByVal sender As Object, ByVal e As EventArgs) Handles button1.Click
            If IsValidated() Then
                If (Not CheckBox1.Checked) Then
                    ...
                    Query = "SELECT Job_Id, Job_Class, Job_Num, Job_Price,Job_Name FROM(Orders) WHERE Cus_Id=" &
                            Convert.ToInt32(cbCusname.SelectedValue) & " AND Job_Status='ค้างชำระ'"
                    Fill_Report(cbCusname.Text, _dbconnect.GetAddress(CInt(cbCusname.SelectedValue)),
                                DateTime.Now.ToString("dd/MM/yyyy"),
                                _dbconnect.GetSumBill(Convert.ToInt32(cbCusname.SelectedValue)),
                                _dbconnect.GetEmployeeName())

                Else

                    Query = "SELECT Job_Id, Job_Class, Job_Num, Job_Price,Job_Name FROM(Orders) WHERE Cus_Id=" &
                            Convert.ToInt32(cbCusname.SelectedValue) & " AND Job_Date LIKE '%" &
                            DateTimePicker1.Value.ToString("dd/MM/yyyy") & "%' AND Job_Status='ค้างชำระ'"
                    Fill_Report(cbCusname.Text, _dbconnect.GetAddress(CInt(cbCusname.SelectedValue)),
                                DateTime.Now.ToString("dd/MM/yyyy"),
                                _dbconnect.GetSumBill(Convert.ToInt32(cbCusname.SelectedValue),
        ...
        Private Sub Fill_Report(ByVal cusName As String, ByVal address As String, ByVal rdate As String,
                                ByVal thatbaht As String, ByVal employee As String)
        ...
        Private Sub FrmBill_Activated(sender As Object, e As EventArgs) Handles MyBase.Activated
            If Not CusId = 0 And Query <> "" Then
                cbCusname.SelectedValue = CusId
                Fill_Report(cbCusname.Text, _dbconnect.GetAddress(CInt(cbCusname.SelectedValue)),
                            DateTime.Now.ToString("dd/MM/yyyy"), _dbconnect.GetSumBill(Queryb),
                            _dbconnect.GetEmployeeName())
            End If
        End Sub
    End Class
